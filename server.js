...
// Анти-спам (опционально, если rate-limit не установлен - закомментировать)
...
// ========== БАЗЫ ДАННЫХ (JSON) - ПРОСТЫЕ, БЕЗ BCYPT ==========
...
if (fs.existsSync(roomsFile)) {
    ...
    for (let [name, data] of Object.entries(roomsData)) {
        if (rooms[name]) rooms[name].messages = data.messages.slice(-500);
...
function sendSystemMessage(room, text) {
    ...
    if (rooms[room].messages.length > 1000) rooms[room].messages = rooms[room].messages.slice(-500);
    saveRooms();
}

function sendGlobalAnnouncement(text) {
    for (let room in rooms) sendSystemMessage(room, text);
}
...
function startFactTimer() {
    ...
    factInterval = setInterval(() => {
        ...
        for (let room in rooms) sendSystemMessage(room, randomFact);
    }, 2 * 60 * 1000);
}
...
app.post("/register", (req, res) => {
    ...
    usersDB[username] = { password: password, role: "новичок", createdAt: Date.now(), avatar: null, status: "online" };
    saveUsers();
...
server.listen(PORT, () => console.log(`🚀 SWILL сервер на порту ${PORT}`));

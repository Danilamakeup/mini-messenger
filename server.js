...
let rooms = {
    ...
    "voice-chat": { messages: [], users: new Set() }
};
...
const funFacts = [
    ...
    "🎮 Факт: первый в мире чат появился в 1973 году на компьютере PLATO",
    "🐱 Факт: коты не умеют жевать еду — у них нет жевательных зубов",
    "🍕 Факт: самый дорогой пицца в мире стоит $12,000 и украшена золотом",
    "💀 Факт: Discord изначально создавался как игра, но потом стал чатом",
    "🔥 Факт: ты сейчас читаешь этот факт в моём мессенджере — гордись собой!"
];
...
io.on("connection", (socket) => {
    ...
    socket.on("auth", ({ username, room }) => {
        ...
        // Запускаем таймер интересных фактов для первой комнаты
